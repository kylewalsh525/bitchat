...
struct ViewSmokeTests {
    ...
    func geohashAndTextMessageViews_renderCoreBranches() {
        ...
        _ = mount(TextMessageView(message: truncatableMessage, showStreamingIndicator: false, expandedMessageIDs: .constant([])).environmentObject(viewModel))
        _ = mount(TextMessageView(message: paymentMessage, showStreamingIndicator: false, expandedMessageIDs: .constant([])).environmentObject(viewModel))
