...
private final class MockCommandContextProvider: CommandContextProvider {
    ...
    var agentRuntimeStatus = AgentRuntimeStatus()
    ...
    private(set) var dispatchedAgentRequests: [(role: String, prompt: String)] = []
    private(set) var handledAgentSessionCommands: [String] = []
    private(set) var updatedAgentConfigs: [AgentConfig] = []
    ...
    func dispatchAgentRequest(role: String, prompt: String) -> CommandResult {
        dispatchedAgentRequests.append((role, prompt))
        return .handled
    }

    func handleAgentSessionCommand(_ args: String) -> CommandResult {
        handledAgentSessionCommands.append(args)
        return .handled
    }

    func updateAgentConfig(_ config: AgentConfig) {
        agentConfig = config
        updatedAgentConfigs.append(config)
    }
